...
public interface UserFCMTokenRepository extends JpaRepository<UserFCMToken, Long> {
    ...
    /**
     * 사용자 ID로 활성화되고 알림이 허용된 모든 디바이스 조회
     * @param userId 사용자 ID
     * @return 활성화되고 알림이 허용된 디바이스 리스트
     */
    @Query("SELECT t FROM UserFCMToken t WHERE t.userId = :userId AND t.isActive = true AND t.isAllowed = true")
    ...
    /**
     * 사용자 ID와 디바이스 모델로 토큰 조회
     * @param userId 사용자 ID
     * @param deviceModel 디바이스 모델
     * @return FCM 토큰 (Optional)
     */
    Optional<UserFCMToken> findByUserIdAndDeviceModelAndIsActiveTrue(Long userId, String deviceModel);
